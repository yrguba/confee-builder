...
function getMessageMenuItems(message: MessageProxy): MessageMenuItem[] {
    ...
    // remove copy item
    if (message.type !== 'text' || message.forwarded_from_messages?.length) {
        items.splice(findIndex(2), 1);
    }
    // remove Edit item
    if (!message.isMy || moment().unix() - moment(message.created_at).unix() > 86400 || message.forwarded_from_messages?.length || message.type !== 'text') {
    ...
    // remove task item
    if (message.forwarded_from_messages?.length) {
